...
export default function Profile() {
  ...
  const LoginFound = users.find((items) => items.name === loginUser);
      ...
      {LoginFound ? (
        <>
          <div className="rightSide rounded">
            ...
            <h1 className="mt-3 text-center h4">{LoginFound.name}</h1>
          </div>
          ...
          <div className="midSection rounded">
            ...
            {/* {LoginFound.posts.map((item, index) => (
              <div key={index} className="post border border-2 border-dark rounded mt-3 px-4 py-2"> {date.getDate()},{getMonthName(0)},{date.getFullYear()}
